import { VscError, VscWarning, VscInfo, VscClose, VscRefresh } from "react-icons/vsc";
    ...
    >
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          <h3 className={`font-medium ${config.titleClass}`}>{title}</h3>
          {message && (
...
// Git-specific errors
export function GitError({ error, onRetry }: { error: string; onRetry?: () => void }) {
  return (
    <ErrorDisplay
      severity="error"
      title="Git Error"
...
export function TmuxError({ error, onRetry }: { error: string; onRetry?: () => void }) {
  return (
    <ErrorDisplay
      severity="error"
      title="tmux Error"
