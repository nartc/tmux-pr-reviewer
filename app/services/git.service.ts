  ...
  GitService.of({
    ...
    getInfo: (repoPath: string) =>
      Effect.gen(function* () {
        ...
        if (!isRepo) {
          return yield* Effect.fail(new NotAGitRepoError({ path: repoPath }));
        }

        const [remoteUrl, currentBranch, baseBranch] = yield* Effect.all([
          Effect.tryPromise({
            try: async () => {
              const remotes = await git.getRemotes(true);
              const origin = remotes.find((r) => r.name === "origin");
              ...
              new GitError({ message: "Failed to get remote", cause: error }),
          }),
          Effect.tryPromise({
            try: async () => {
              const branch = await git.revparse(["--abbrev-ref", "HEAD"]);
              ...
              new GitError({ message: "Failed to get branch", cause: error }),
          }),
          Effect.tryPromise({
            try: async () => {
              try {
      ...
      Effect.gen(function* () {
        ...
        const diffSummary = yield* Effect.tryPromise({
          try: () => git.diffSummary([baseBranch]),
            ...
            new GitError({ message: "Failed to get diff summary", cause: error }),
        });
        ...
        const rawDiff = yield* Effect.tryPromise({
          try: () => git.diff([baseBranch]),
            ...
            new GitError({ message: "Failed to get raw diff", cause: error }),
        });
        ...
        const files: DiffFile[] = diffSummary.files.map((file) => {
          ...
          return {
            path: file.file,
            status: parseStatus(
              isBinary ? "M" : textFile.insertions > 0 && textFile.deletions === 0 ? "A" : textFile.deletions > 0 && textFile.insertions === 0 ? "D" : "M"
            ),
...
export const createGitService = () => {
  return {
    ...
    getDiffSummary: async (path: string, baseBranch: string) => {
      ...
      // Merge the summaries
      const fileMap = new Map<string, typeof committedSummary.files[0]>();
      ...
      return {
        ...
        insertions: committedSummary.insertions + uncommittedSummary.insertions,
        deletions: committedSummary.deletions + uncommittedSummary.deletions,
