...
export const tmuxService = {
	...
	getSessionWindows: async (sessionName: string): Promise<TmuxWindow[]> => {
		const service = makeTmuxService();
		return Effect.runPromise(
			service
	...
	sendToSession: async (sessionName: string, text: string): Promise<void> => {
		const service = makeTmuxService();
		return Effect.runPromise(
			service.sendToSession(sessionName, text).pipe(
	...
	formatComment: (
		...
		return makeTmuxService().formatComment(filePath, lineStart, content);
	},
	...
	formatComments: (
		...
		return makeTmuxService().formatComments(comments);
	},
	...
	sendComment: async (
		...
		const service = makeTmuxService();
		return Effect.runPromise(
			service.sendComment(sessionName, filePath, lineStart, content).pipe(
				Effect.catchAll((error) => {
	...
	sendComments: async (
		...
		const service = makeTmuxService();
		return Effect.runPromise(
			service.sendComments(sessionName, comments).pipe(
				Effect.catchAll((error) => {
	...
	getWindowsForRepoPath: (
		...
		return makeTmuxService().getWindowsForRepoPath(windows, repoPath);
	},
};
