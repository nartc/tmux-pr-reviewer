...
const LoggingLive = Layer.mergeAll(Logger.replace(Logger.defaultLogger, Logger.make(({ logLevel, message, annotations, date }) => {
    ...
    const annotationStr = Object.keys(annotations).length
        ...
        : '';
    console.error(`[${date.toISOString()}] ${level}: ${message}${annotationStr}`);
})), Logger.minimumLogLevel(LogLevel.Info));
// App layer with all services - proper dependency chain
const AppLayer = DbServiceLive.pipe(Layer.provideMerge(McpConfigLive), Layer.provide(LoggingLive));
// Runtime instance
...
const getOrCreateClient = Effect.gen(function* () {
    ...
    if (currentClientId) {
        // Update last seen
        yield* db
            .execute(`UPDATE mcp_clients SET last_seen_at = datetime('now') WHERE id = ?`, [currentClientId])
    ...
    yield* db
        .execute(`INSERT INTO mcp_clients (id, client_name, working_dir, connected_at, last_seen_at)
			 VALUES (?, ?, ?, datetime('now'), datetime('now'))`, [currentClientId, config.clientName, config.workingDir])
        .pipe(Effect.catchAll(() => Effect.succeed({ changes: 0 })));
    yield* Effect.logInfo('MCP client connected').pipe(Effect.annotateLogs({
...
server.tool('check_pr_comments', 'Check for pending PR review comments in the current repository. Returns comments that have been sent from the PR Reviewer UI and marks them as delivered.', {
    repo_path: z
        .string()
        .optional()
        .describe('Optional: Repository path (auto-detected from cwd if not provided)'),
}, async ({ repo_path }) => {
...
server.tool('mark_comment_resolved', 'Mark a PR review comment as resolved after addressing it. Use the comment ID from check_pr_comments.', {
    ...
    const program = Effect.gen(function* () {
        ...
        if (error._tag === 'CommentNotFoundError') {
            return `Comment not found: ${error.id}`;
        }
        return `Error: ${message}`;
...
server.tool('list_pending_comments', 'List pending PR review comments across all registered repositories. Shows a summary of undelivered comments per repository.', {}, async () => {
    const program = Effect.gen(function* () {
        yield* getOrCreateClient;
...
server.tool('list_repo_pending_comments', 'List pending PR review comments for the current repository only. Shows a summary grouped by file.', {
    repo_path: z
        .string()
        .optional()
        .describe('Optional: Repository path (auto-detected from cwd if not provided)'),
...
server.tool('get_comment_details', 'Get detailed information about a specific PR review comment, including file path, line numbers, content, and status.', {
