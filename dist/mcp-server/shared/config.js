...
export const McpConfig = Context.GenericTag('McpConfig');
// Detect client name from environment using Config
const detectClientName = Effect.gen(function* () {
    for (const { key, name } of clientEnvVars) {
        const value = yield* Config.string(key).pipe(Config.withDefault(''));
        if (value)
            return name;
    }
...
const makeConfig = Effect.gen(function* () {
    ...
    const workingDir = yield* Config.string('PWD').pipe(Config.orElse(() => Config.succeed(process.cwd())));
...
export const McpConfigLive = Layer.effect(McpConfig, makeConfig.pipe(Effect.tap((config) => Effect.logDebug('MCP Config initialized', {
    workingDir: config.workingDir,
    clientName: config.clientName,
})), Effect.orDie));
// For testing - create config with custom values
export const makeTestConfig = (overrides) => ({
...
export const McpConfigTest = (overrides = {}) => Layer.succeed(McpConfig, makeTestConfig(overrides));
