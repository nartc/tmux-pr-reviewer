...
interface RepoCommentCount {
	repo_name: string;
	repo_path: string;
	pending_count: number;
}

export const listPending = (): Effect.Effect<
	string,
	DatabaseError,
	DbService
> =>
	Effect.gen(function* () {
		...
		const results = yield* db.query<RepoCommentCount>(
			`SELECT 
			   r.name as repo_name,
		...
		lines.push('');
		lines.push(
			`Total: ${totalPending} pending comment${totalPending === 1 ? '' : 's'}`,
		);
