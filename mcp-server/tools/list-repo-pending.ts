...
export const listRepoPending = (
	args: ListRepoPendingArgs,
): Effect.Effect<
	string,
...
> =>
	Effect.gen(function* () {
		...
		const comments = yield* db.query<Comment>(
			`SELECT c.*
			 FROM comments c
			 WHERE c.session_id = ?
			   AND c.status = 'sent'
		...
		for (const [filePath, fileComments] of byFile) {
			lines.push(
				`📄 ${filePath} (${fileComments.length} comment${fileComments.length === 1 ? '' : 's'})`,
			);
		...
		lines.push(
			`Total: ${comments.length} pending comment${comments.length === 1 ? '' : 's'}`,
		);
		lines.push('');
